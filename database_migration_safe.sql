-- Safe Database Migration for Road to 1% - Comprehensive SPI Assessment
-- This script handles existing objects gracefully and only adds missing RLS policies
-- Run this in your Supabase SQL Editor

-- Create user_tier enum type (only if it doesn't exist)
DO $$ BEGIN
...
-- Create assessment_period enum type (only if it doesn't exist)
DO $$ BEGIN
...
-- Create user_progress table (only if it doesn't exist)
...
-- Create comprehensive_assessments table (only if it doesn't exist)
...
-- Create quarterly_assessments table (only if it doesn't exist)
...
-- Create assessment_reminders table (only if it doesn't exist)
...
-- Create action_plans table (only if it doesn't exist)
...
-- Create progress_milestones table (only if it doesn't exist)
...
-- Create RLS policies for user_progress table (only if they don't exist)
DO $$ BEGIN
...
-- Create RLS policies for comprehensive_assessments table (only if they don't exist)
DO $$ BEGIN
...
-- Create RLS policies for quarterly_assessments table (only if they don't exist)
DO $$ BEGIN
...
-- Create RLS policies for assessment_reminders table (only if they don't exist)
DO $$ BEGIN
...
-- Create RLS policies for action_plans table (only if they don't exist)
DO $$ BEGIN
...
-- Create RLS policies for progress_milestones table (only if they don't exist)
DO $$ BEGIN
...
-- Create function to calculate next quarterly assessment date (only if it doesn't exist)
...
-- Create function to get assessment period from date (only if it doesn't exist)
...
-- Success message
SELECT 'Database migration completed successfully! RLS policies are now in place.' as status; 